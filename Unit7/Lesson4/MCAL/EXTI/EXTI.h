...
typedef struct
{
	...
	uint16		EXTI_InputLineNumber;	// Choose The Line Number To Use in EXTI

	GPIO_Typedef_t* 	GPIOx;					// GPIO Port Used For EXTI Driver

	uint16		GPIO_Pin;				// GPIO Pin Used For EXTI Driver

	uint8			IVT_IRQ_Number;			// Choose Interrupt Handler From Interrupt Vector Table
}EXTI_GPIO_Mapping_t;
...
typedef struct
{
	...
	uint8					Trigger_Case;		// Specifics RISING or FALLING or Both trigger
												// This parameter must be set based on @ref EXTI_Trigger_define

	uint8					IRQ_Enable;			// Enable or Disable the EXTI IRQ (That will enable the IRQ mask in EXTI and also on the NVIC Interrupt contoller)
